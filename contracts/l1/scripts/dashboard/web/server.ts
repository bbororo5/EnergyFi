 ...
 *
 * 실행: npm run dashboard:web:testnet
 *      npm run dashboard:web:local
 *
...
export interface ContractCtx {
  ...
  deviceRegistry: ethers.Contract;
  stationRegistry: ethers.Contract;
  // Phase 2 (optional — 미배포 시 undefined)
  chargeTransaction?: ethers.Contract;
  revenueTracker?: ethers.Contract;
  chargeRouter?: ethers.Contract;
}
...
function getRpcUrl(network: string): string {
  ...
  if (!url) throw new Error(`알 수 없는 네트워크: ${network}`);
  if (!url) throw new Error(`RPC URL 미설정. .env의 ENERGYFI_L1_TESTNET_RPC를 확인하세요.`);
  return url;
}
...
async function main() {
  ...
  if (addrs.ChargeTransaction && addrs.RevenueTracker && addrs.ChargeRouter) {
    ...
    if (ctAbi && rtAbi && crAbi) {
      ...
      console.log(`⚠️ Phase 2 ABI 미발견 — npm run compile 후 재시도`);
    }
  } else {

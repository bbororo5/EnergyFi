...
import { BaseContract, encodeBytes32String, keccak256, getBytes, solidityPacked } from "ethers";
 ...
 */
export async function deployUUPSProxy<T extends BaseContract>(
  ethers: any,
  contractName: string,
): Promise<{ impl: BaseContract; proxy: BaseContract; contract: T }> {
  ...
  const contract = (await ethers.getContractAt(contractName, await proxy.getAddress())) as unknown as T;

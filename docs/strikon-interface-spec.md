...
> **참고**: 온체인 인터페이스(스마트컨트랙트, Blockchain Bridge 등)는 본 문서 범위 밖이며, EnergyFi 아키텍처 로드맵(`contracts/docs/implementation-roadmap.md`)을 참조할 것.
...
> `invoice.paid` 이벤트가 EnergyFi 온체인 레이어의 입력 트리거가 된다. Blockchain Bridge는 수신 후 `ChargeRouter.processCharge()`를 호출하고, ChargeRouter가 (1) `ChargeTransaction.mint()` → (2) `RevenueTracker.recordRevenue()`를 단일 TX로 원자적 처리한다. 이후 상세 흐름은 EnergyFi 컨트랙트 명세(`contracts/docs/phase2-transaction-spec.md`) 참조.
...
| `station_id` | `UUID v4` | ✓ | 스테이션 ID. Blockchain Bridge가 `ChargeRouter.processCharge()` 호출 시 사용. StationRegistry에서 등록 여부 검증. |
| `region_id`  | `string`  | ✓ | 행정구역 코드 (ISO 3166-2:KR). 예: `"KR-11"` (서울). Bridge가 두 가지 목적으로 사용: (1) STO 수익 풀 귀속 지역 결정 (`RevenueTracker.getRegionRevenue()`) (2) Phase 5 탄소 계산 시 지역별 전력 배출 계수(EFkw) 조회. |
...
| `pg_provider` | `enum` | ✓ | `INICIS` \| `KAKAOPAY` \| `NAVERPAY` \| `TOSS` \| `KSNET`. STRIKON 내부 라우팅. |
...
> `invoice.paid`는 EnergyFi 온체인 레이어의 입력 트리거 이벤트다. Blockchain Bridge는 이 이벤트 수신 시 `ChargeRouter.processCharge(session, period_yyyyMM)`를 호출하며, ChargeRouter가 (1) `ChargeTransaction.mint(session)` → (2) `RevenueTracker.recordRevenue(stationId, distributableKrw, period_yyyyMM)`를 단일 TX로 원자적 처리한다.
...
| `region_id` | `string` | ✓ | 행정구역 코드 (ISO 3166-2:KR). Bridge가 두 가지 목적으로 사용: (1) STO 수익 풀 귀속 지역 결정 (2) Phase 5 탄소 EFkw 조회. |
| `cpo_id` | `UUID v4` | ✓ | CPO ID. **온체인에 전달되지 않음** — STRIKON 오프체인 정산 전용. Bridge가 이 필드를 무시하고 온체인 `ChargeSession` struct에 포함하지 않음. |
| `se_signature` | `bytes (hex)` | ✓ | TPM 2.0 SE 칩 P-256 서명. STRIKON이 session.stopped → invoice.paid 경로로 전파. Bookend 신뢰 모델 필수 필드. Bridge가 ChargeRouter.processCharge()를 통해 전달하며, ChargeTransaction.mint()에서 검증 후 스토리지 저장 제외·이벤트(`ChargeSessionRecorded`)로만 보존. |

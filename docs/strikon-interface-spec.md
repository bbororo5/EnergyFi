...
| `region_id`  | `string`  | ✓ | 행정구역 코드 (ISO 3166-2:KR). 예: `"KR-11"` (서울). Bridge가 두 가지 목적으로 사용: (1) STO 수익 풀 귀속 지역 결정 (`RevenueTracker.getRegionRevenue()`) (2) Phase 4 탄소 계산 시 지역별 전력 배출 계수(EFkw) 조회. |
...
| `region_id` | `string` | ✓ | 행정구역 코드 (ISO 3166-2:KR). Bridge가 두 가지 목적으로 사용: (1) STO 수익 풀 귀속 지역 결정 (2) Phase 4 탄소 EFkw 조회. |

...
export const phase1InfraSuite: TestSuite = {
  ...
  async run(ctx: ContractCtx, emit: EmitFn): Promise<Counts> {
    ...
    const ifaces = [dr.interface, sr.interface];
    ...
    await expectSuccess("1-1 SE칩 등록 성공",
      () => dr.enrollChip(CHIP_ID_1, PUB_KEY_64, 1),
      emit, counts);

    // 1-2. isActiveChip → true 확인
    ...
    await expectSuccess("2-1 CPO 등록 성공",
      () => sr.registerCPO(CPO_ID_1, CPO_WALLET, "통합테스트 CPO-1"),
      emit, counts);

    // 2-2. getCPO 이름·지갑 검증
    await expectValue("2-2 getCPO 이름·지갑 검증",
      () => sr.getCPO(CPO_ID_1),
      (cpo: any) => {
        if (cpo.name === "통합테스트 CPO-1" && cpo.walletAddress === CPO_WALLET) return true;
        return `name="${cpo.name}", wallet=${cpo.walletAddress}`;
      },
      emit, counts);

    // 2-3. 중복 CPO 차단
    await exp("2-3 중복 CPO 차단",
      () => sr.registerCPO(CPO_ID_1, CPO_WALLET, "중복 CPO"),
      "CPOAlreadyExists");

    // 2-4. zero wallet address 차단 (R07)
    await exp("2-4 zero wallet address 차단",
      () => sr.registerCPO(b32(`PZ-${ts}`), ethers.ZeroAddress, "제로 지갑 CPO"),
      "ZeroWalletAddress");
    ...
    await expectSuccess("3-1 CPO-owned 충전소 등록",
      () => sr.registerStation(STN_CPO, CPO_ID_1, OWNER_CPO, REGION_KR11, "서울 강남구 APT-A"),
      emit, counts);

    // 3-2. ENERGYFI-owned 충전소 등록
    await expectSuccess("3-2 ENERGYFI-owned 충전소 등록",
      () => sr.registerStation(STN_EFI, ethers.ZeroHash, OWNER_EFI, REGION_KR11, "서울 성동구 APT-B"),
      emit, counts);

    // 3-3. 미등록 CPO 차단
    await exp("3-3 미등록 CPO 차단",
      () => sr.registerStation(b32(`SX-${ts}`), CPO_ID_GHOST, OWNER_CPO, REGION_KR11, "없는 CPO"),
      "CPONotFound");

    // 3-4. ENERGYFI + regionId 없음 차단
    await exp("3-4 ENERGYFI + regionId 없음 차단",
      () => sr.registerStation(b32(`SZ-${ts}`), ethers.ZeroHash, OWNER_EFI, REGION_EMPTY, "리전 없음"),
      "RegionRequired");

    // 3-5. CPO + cpoId 없음 차단
    await exp("3-5 CPO + cpoId 없음 차단",
      () => sr.registerStation(b32(`SW-${ts}`), ethers.ZeroHash, OWNER_CPO, REGION_KR11, "CPO ID 없음"),
      "CpoRequired");

    // 3-6. 비활성 CPO 차단
    const cpo2Ready = await setup("CPO-2 등록 후 비활성화", async () => {
      const tx1 = await sr.registerCPO(CPO_ID_2, CPO_WALLET, "통합테스트 CPO-2");
      await tx1.wait();
      const tx2 = await sr.deactivateCPO(CPO_ID_2);
      await tx2.wait();
    }, emit, counts);
    if (cpo2Ready) {
      await exp("3-6 비활성 CPO 차단",
        () => sr.registerStation(b32(`SY-${ts}`), CPO_ID_2, OWNER_CPO, REGION_KR11, "비활성 CPO"),
        "CPONotActive");
    }

    // 3-7. 중복 stationId 차단 (신규)
    await exp("3-7 중복 stationId 차단",
      () => sr.registerStation(STN_CPO, CPO_ID_1, OWNER_CPO, REGION_KR11, "중복 충전소"),
    ...
    await expectSuccess("4-1 충전기 등록 성공",
      () => sr.registerCharger(CHG_ID_1, STN_CPO, 0),
      emit, counts);

    // 4-2. 미등록 충전소 차단
    await exp("4-2 미등록 충전소 차단",
      () => sr.registerCharger(CHG_NO_STN, b32(`NS-${ts}`), 1),
      "StationNotFound");

    // 4-3. 중복 충전기 차단
    await exp("4-3 중복 충전기 차단",
      () => sr.registerCharger(CHG_ID_1, STN_CPO, 1),
      "ChargerAlreadyExists");

    // 4-4. 비활성 충전소 차단
    const stnReady = await setup("STN_INACTIVE 등록 후 비활성화", async () => {
      const tx1 = await sr.registerStation(STN_INACTIVE, CPO_ID_1, OWNER_CPO, REGION_KR11, "비활성화될 충전소");
      await tx1.wait();
      const tx2 = await sr.deactivateStation(STN_INACTIVE);
      await tx2.wait();
    }, emit, counts);
    if (stnReady) {
      await exp("4-4 비활성 충전소 차단",
        () => sr.registerCharger(CHG_INACT, STN_INACTIVE, 1),
        "StationNotActive");
    }

    // 4-5. SE칩 미등록 충전기 등록 차단 (gap 해결됨)
    await exp("4-5 SE칩 미등록 충전기 등록 차단",
      () => sr.registerCharger(CHG_NOGAP, STN_CPO, 1),
      "ChipNotActive");

    // 4-6. 유효하지 않은 chargerType 차단 (신규)
    await exp("4-6 유효하지 않은 chargerType(99) 차단",
      () => sr.registerCharger(b32(`GT-${ts}`), STN_CPO, 99),
      "InvalidChargerType");

    // ── 5. 계층 구조 조회 (5-1 ~ 5-5) ─────────────────────────────────────

    // 5-1. getStationsByCPO 확인
    ...
    // 5-2. getChargersByStation 확인
    await expectValue("5-2 getChargersByStation 확인",
      () => sr.getChargersByStation(STN_CPO),
      (chgs: string[]) => chgs.includes(CHG_ID_1) || `목록에 없음 (${chgs.length}개)`,
      emit, counts);

    // 5-3. isEnergyFiOwned CPO=false, EFI=true
    ...
    // 5-4. getEnergyFiStationsByRegion EFI만 반환
    ...
    // 5-5. getStationsByRegion 전체 반환 (신규)
    await expectValue("5-5 getStationsByRegion 전체 반환",
      () => sr.getStationsByRegion(REGION_KR11),
      (stns: string[]) => {
        const hasCpo = stns.includes(STN_CPO);
        const hasEfi = stns.includes(STN_EFI);
        if (hasCpo && hasEfi) return true;
        return `CPO=${hasCpo}, EFI=${hasEfi}`;
      },
      emit, counts);
    ...
    await exp("6-1 충전기 있는 충전소 비활성화 차단",
      () => sr.deactivateStation(STN_CPO),
      "HasActiveChargers");

    // 6-2. 충전소 있는 CPO 비활성화 차단
    await exp("6-2 충전소 있는 CPO 비활성화 차단",
      () => sr.deactivateCPO(CPO_ID_1),
      "HasActiveStations");

    // 6-3. 충전기 비활성화 후 충전소 비활성화 성공 (신규)
    // 먼저 EFI 충전소에 충전기를 등록한 후 비활성화 테스트
    const chg2Ready = await setup("EFI 충전소에 충전기 등록", async () => {
      const tx = await sr.registerCharger(CHG_ID_2, STN_EFI, 0);
      await tx.wait();
    }, emit, counts);
    if (chg2Ready) {
      await expectSuccess("6-3 충전기 비활성화 후 충전소 비활성화 성공",
        async () => {
          const tx1 = await sr.deactivateCharger(CHG_ID_2);
          await tx1.wait();
          const tx2 = await sr.deactivateStation(STN_EFI);
          return tx2;
        },
        emit, counts);
    }

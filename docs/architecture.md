...
| **StationRegistry** | Station → Charger hierarchy. regionId 매핑. 모든 온체인 충전소는 EnergyFi 소유 | Phase 1 (2026.04) |
...
| **RevenueTracker** | 결제 완료 건 수익 누적. 충전소별·지역별 수익 집계 (전액 STO 투자자 풀) | Phase 2 (2026.06) |
...
| **ReputationRegistry** | Station operational quality scores (oracle pattern, 선택) | Phase 3 (2027.01~, 보류) |
...
> **CPO 정책 변경 (2026.03)**: 제3자 CPO는 STRIKON 소프트웨어만 사용하며 온체인에 등록되지 않음. 모든 온체인 충전소는 EnergyFi 소유. StationRegistry에서 `OwnerType`, `CPORecord`, `cpoId` 필드 제거됨.
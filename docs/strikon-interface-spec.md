...
> `invoice.paid` 이벤트가 EnergyFi 온체인 레이어의 입력 트리거가 된다. Blockchain Bridge는 수신 후 (1) `ChargeTransaction.mint()` → (2) `RevenueTracker.recordRevenue()`를 연속 호출한다. 이후 상세 흐름은 EnergyFi 컨트랙트 명세(`contracts/docs/phase2-transaction-spec.md`) 참조.

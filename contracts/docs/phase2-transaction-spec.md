...
STRIKON `invoice.paid` 이벤트 발생 시 충전 데이터를 온체인에 영구 기록하고, 충전소별·지역별 수익을 실시간으로 누적·추적한다.
...
| 10 | Setup 데이터 규모 | **Phase 1 배치와 동일** (충전소 5 + 충전기 15 + SE칩 12 + 3개 지역) | Mock Oracle Setup |
...
**목적**: ChargeRouter.processCharge() 내에서 mint() 직후 호출. 충전소별·지역별 수익을 누적하고 집계를 제공. Phase 3 STOPortfolio의 데이터 소스. 모든 온체인 충전소는 EnergyFi 소유이므로 수익은 전액 해당 지역 STO 투자자 풀로 귀속.
...
// StationRegistry.getStationsByRegion(regionId)로 충전소 목록 조회
...
//   5. emit SettlementRecorded(sid, pending, period_yyyyMM) per station
//   6. emit RegionClaimed(regionId, totalClaimed, period_yyyyMM)
// require: totalClaimed > 0 ("NothingToClaim")

// View — 페이지네이션 버전
function getRegionRevenuePaginated(bytes4 regionId, uint256 offset, uint256 limit)
    external view
...
- **StationRegistry**: `getRegionRevenue()` 에서 충전소 목록 조회. `recordRevenue()` 시 stationId 유효성 검증.
...
```
1. 접두사 제거 (sess_, chrg_, stn_, inv_ 등)
2. 하이픈(-) 제거
...
| `region_id` (ISO 3166-2:KR) | `gridRegionCode` (bytes4) | 탄소 EFkw 조회용. |
| `cpo_id` (UUID) | — (온체인 전달 안 함) | STRIKON 오프체인 전용. Bridge가 온체인에 전달하지 않음. |
...
- **`gridRegionCode`**: 모든 세션에 기록. Phase 4 탄소 EFkw(전력 배출 계수) 조회용.
- **`StationRegistry.regionId`**: 충전소 등록 시 필수. STO 수익 풀 귀속 지역 결정.
...
| 3 | POST | `/oracle/phase2/claim` | 지역 월정산 (body: `{ regionId, period }`) |
...
| # | 탭 이름 | 출처 | 설명 |
|:--|:--|:--|:--|
| 1 | 인프라 현황 | Phase 1 | 지역별 충전소 + SE 칩 상태 |
| 2 | Oracle Phase 1 | Phase 1 | 충전소/충전기/SE칩 등록 + 전체 배치 |
| 3 | 실시간 충전 피드 | Phase 1 scaffold, **Phase 2 활성화** | SSE 기반 실시간 세션 피드 + 최근 N건 |
| 4 | 수익 현황 | Phase 1 scaffold, **Phase 2 활성화** | 충전소별·지역별·월별 수익 통계 |
...
GET /events — SSE 스트림
  이벤트 타입:
    ...
    4. claim      — RevenueTracker.RegionClaimed 이벤트
...
| GET `/query/phase2/region-revenue` | 지역별 수익 | `RevenueTracker.getRegionRevenue()` |
...
│                                                                 │
│  시간           충전소          kWh    금액(원)  소유          │
│  14:23:01  STATION-003 (서울)  45.2   13,560  ✅               │
│  14:19:45  STATION-001 (서울)  22.8    6,840  ✅               │
│  14:15:30  STATION-005 (경기)  61.0   18,300  ✅               │
│  14:10:12  STATION-004 (서울)  38.5   11,550  ✅               │
│  14:08:55  STATION-002 (부산)  19.0    5,700  ✅               │
└─────────────────────────────────────────────────────────────────┘
```
...
│  충전소별 수익 현황                                               │
│                                                                 │
│  충전소          지역     누적(원)    정산(원)    대기(원)       │
│  STATION-001    서울     125,400    125,400         0          │
│  STATION-002    부산      87,600     87,600         0          │
│  STATION-003    서울     342,000    300,000     42,000         │
│  STATION-004    서울     198,500    150,000     48,500         │
│  STATION-005    경기     445,200    400,000     45,200         │
└─────────────────────────────────────────────────────────────────┘
```
...
│  지역별 수익 통계 (STO 투자자 풀)                                  │
│                                                                 │
│  서울(KR11)    이번 달: 245,600원  전달: 665,900원               │
│  경기(KR41)    이번 달: 132,100원  전달: 445,200원               │
│  부산(KR26)    이번 달:  52,300원  전달:  87,600원               │
└─────────────────────────────────────────────────────────────────┘
```
...
│  총 세션 수:   1,248건                                           │
│  총 에너지:    34,890 kWh                                        │
│  총 수익(원):  10,467,000원 (전액 STO 투자자 풀)                   │
└─────────────────────────────────────────────────────────────────┘
```
...
│                                                                 │
│  충전소        지역          기간      금액(원)    정산일시       │
│  STATION-001  서울(KR11)    202603  125,400  2026-04-01 10:00│
│  STATION-002  부산(KR26)    202603   87,600  2026-04-01 10:05│
│  STATION-003  서울(KR11)    202603  342,000  2026-04-01 11:00│
│  ...                                                            │
│                                                                 │
...
│                                                                 │
│  데이터 소스: RevenueTracker.getRegionRevenue(regionId)          │
└─────────────────────────────────────────────────────────────────┘
```
...
- [ ] `claimRegion(regionId, period_yyyyMM)` 후 각 충전소 pending = 0 검증 (전액 정산 확인)
- [ ] `claimRegion()` 재호출 시 `NothingToClaim` revert 확인
- [ ] `SettlementRecorded` 이벤트 충전소 수만큼 emit 확인
- [ ] `RegionClaimed` 이벤트 totalAmount = 모든 충전소 pending 합산 값 확인
- [ ] `getStationRevenuePeriod()` 월별 통계 쿼리 정확성 검증
- [ ] `getRegionRevenue()` StationRegistry 연동 확인
- [ ] `onlyBridge` 권한 테스트 → 다른 주소 revert 확인
...
| 3. 지역 월정산 | RevenueTracker: claimRegion, SettlementRecorded, RegionClaimed |
| 4. 충전소별 수익 | RevenueTracker: getStationRevenue, getStationRevenuePeriod |
| 5. 지역 수익 | RevenueTracker: getRegionRevenue |
...
| `RevenueTracker.getRegionRevenue()` | 탭 4 (지역별 수익 통계) |
...
├── routes/
│   ├── query.ts                     # Phase 1 GET (지역 조회, 충전소 목록)

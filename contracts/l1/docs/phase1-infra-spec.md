...
충전기 SE 칩 공개키 등록, CPO·충전소·충전기 계층 구조 온체인 확립. CPO 소유 vs EnergyFi 소유 충전소 구분을 통해 Phase 2 수익 귀속의 기반 데이터를 구축한다.
...
**현재 구현 상태**: `contracts/l1/contracts/DeviceRegistry.sol` 배포 완료.
...
**목적**: CPO → Station → Charger 계층 구조 관리. 각 Station에 ownerType(CPO/ENERGYFI)을 부여하여 Phase 2 수익 귀속의 근거 데이터를 제공한다.

#### ownerType 비즈니스 의미

```
CPO 소유(OwnerType.CPO):
  → EnergyFi가 충전기+소프트웨어를 CPO에게 판매
  → CPO가 해당 충전소 소유·운영
  → distributableKrw 100% → CPO (STO 무관)
  → regionId: optional. STO 귀속 불필요. Phase 4 탄소 계산은 ChargeSession.gridRegionCode 사용.

EnergyFi 소유(OwnerType.ENERGYFI):
  → EnergyFi가 직접 소유·운영하는 충전소
...
struct CPORecord {
    bytes32 cpoId;
    address walletAddress;   // CPO 정산 수령 지갑
    string  name;
    bool    active;
}
...
struct Station {
    bytes32   stationId;
    bytes32   cpoId;         // OwnerType.ENERGYFI인 경우 zero
    OwnerType ownerType;
    bytes4    regionId;   // ISO 3166-2:KR bytes4 인코딩 (예: 0x4B523131 = "KR11" = 서울)
                         // EnergyFi 소유: 필수 (bytes4(0) 허용 안 됨)
                         // CPO 소유: optional (bytes4(0) = 미등록)
...
```solidity
// CPO 관리 (onlyAdmin)
registerCPO(bytes32 cpoId, address walletAddress, string calldata name)
deactivateCPO(bytes32 cpoId)
  // require: 소속 active 충전소 없음 ("HasActiveStations")
  // 효과: CPORecord.active = false
getCPO(bytes32 cpoId) → CPORecord memory

// Station 관리 (onlyAdmin)
registerStation(
    bytes32   stationId,
    bytes32   cpoId,         // OwnerType.ENERGYFI면 bytes32(0)
    OwnerType ownerType,
    bytes4    regionId,      // EnergyFi 소유: 필수 (STO 수익 풀 + 탄소 EFkw)
                             // CPO 소유: optional — STO 무관, 탄소 EFkw 용도로만 사용 가능
                             //           미등록 시 bytes4(0), ChargeSession.gridRegionCode로 대체
    string calldata location
)
  // require: ownerType == ENERGYFI → regionId != bytes4(0), "RegionRequired"
  // require: ownerType == CPO     → cpoId != bytes32(0), "CpoRequired"
...
// 조회 (view)
getStationsByCPO(bytes32 cpoId) → bytes32[] stationIds
getStationsByRegion(bytes4 regionId) → bytes32[] stationIds
getEnergyFiStationsByRegion(bytes4 regionId) → bytes32[] stationIds
  // Phase 3 STOPortfolio / RevenueTracker.getEnergyFiRegionRevenue()에서 사용
getChargersByStation(bytes32 stationId) → bytes32[] chargerIds
isEnergyFiOwned(bytes32 stationId) → bool
getStationOwner(bytes32 stationId) → (OwnerType ownerType, address ownerAddress)
  // ownerAddress: CPO인 경우 CPO.walletAddress, ENERGYFI인 경우 address(0)
```
...
event StationRegistered(bytes32 indexed stationId, bytes32 indexed cpoId, OwnerType ownerType, bytes4 regionId);
...
3. CPO 등록 (`registerCPO`)
4. 충전소 등록 (`registerStation` — ownerType 지정 필수)
5. 충전기 등록 (`registerCharger`)
6. DeviceRegistry에 SE 칩 등록 (`enrollChip` — 충전기 출하 전)
7. Phase 2 ChargeTransaction 배포 시 DeviceRegistry 주소 전달
...
```
contracts/l1/scripts/dashboard/
...
│  ┌─────────────────────────────────────────────────────┐     │
│  │ 충전소       소유   CPO     L2  DCFC  상태           │     │
│  │ STATION-001  CPO   삼성EV   2    1    ✅             │     │
│  │ STATION-003  EFI   —        2    2    ✅             │     │
│  │ STATION-004  EFI   —        1    0    ✅             │     │
│  └─────────────────────────────────────────────────────┘     │
│  EnergyFi 소유: 2개 (STO 수익 대상)  CPO 소유: 1개           │
│                                                              │
│  CPO 목록                                                    │
│  ┌─────────────────────────────────────────────────────┐     │
│  │ CPO명       ID        충전소   충전기   상태          │     │
...
- `StationRegistry.getEnergyFiStationsByRegion(regionId)`
- `StationRegistry.getChargersByStation(stationId)`
- `StationRegistry.getStationsByCPO(cpoId)` (CPO 목록)
...
│  │ 충전기 등록          │                                     │
│  │ ID:    [CHARGER-001] │  ┌─────────────────────────────┐   │
│  │ 충전소:[STATION-001] │  │ SE 칩 등록                  │   │
│  │ 타입:  [DCFC ▼]     │  │ 충전기: [CHARGER-001       ] │   │
│  │           [등록]     │  │      [등록 (키쌍 자동 생성)] │   │
│  └──────────────────────┘  └─────────────────────────────┘   │
│                                                              │
│  ┌──────────────────────────────────────────────────────┐    │
│  │  [전체 테스트 데이터 등록 (CPO×3, 충전소×5, 충전기×15, SE칩×12)] │
│  └──────────────────────────────────────────────────────┘    │
│                                                              │
│  로그                                                        │
│  ✅ CPO-001 삼성EV 등록 완료                                  │
│  ✅ STATION-003 (EFI, 서울) 등록 완료                         │
...
| 버튼 | 호출 함수 | 서버 엔드포인트 |
|:---|:---|:---|
| CPO 등록 | `StationRegistry.registerCPO()` | `POST /oracle/cpo` |
...
| 전체 등록 | 1~4 순차 실행 | `POST /oracle/all` |
...
- [ ] CPO 등록 후 `getCPO()` 조회 확인
- [ ] EnergyFi 소유 충전소 등록 후 `getEnergyFiStationsByRegion()` 검증
- [ ] CPO 소유 충전소가 `getEnergyFiStationsByRegion()` 결과에 포함되지 않음을 확인
- [ ] 충전기 등록 후 `getChargersByStation()` 조회 확인
- [ ] `isEnergyFiOwned()` CPO 소유 → false, EnergyFi 소유 → true 확인
- [ ] `getStationOwner()` ownerType + ownerAddress 정확성 확인
...
|:---|:---|
| CPO 등록 | StationRegistry: `registerCPO`, `getCPO()` 조회, `CPOAlreadyExists` revert |
| 충전소 등록 (EFI, regionId 없음) | StationRegistry: `RegionRequired` revert 확인 |
| 충전소 등록 (CPO, cpoId 없음) | StationRegistry: `CpoRequired` revert 확인 |
| 충전소 등록 (CPO, 비활성 CPO) | StationRegistry: `CPONotActive` revert 확인 |
...
| 전체 등록 | 위 전체 + `isEnergyFiOwned()` CPO→false, EFI→true 양쪽 확인 |
...
| `StationRegistry.getEnergyFiStationsByRegion()` | 탭1 — EFI/CPO 소유 구분 카운트 |
| `StationRegistry.getChargersByStation()` | 탭1 — 충전기 타입별 카운트 |
| `StationRegistry.getCPO()` | 탭1 — CPO 목록 |
| `StationRegistry.getStationsByCPO()` | 탭1 — CPO별 충전소 수 |

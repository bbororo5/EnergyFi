...
| 1 | **StationRegistry** | 🏗️ 충전소·충전기 계층 관리 | 충전소 등록·지역 매핑 없이 수익 귀속 불가. RevenueTracker의 필수 참조 대상. Phase 1 배포. |
...
| 3 | **RevenueTracker** | 💰 수익 누적·정산 추적 | ChargeRouter.processCharge() 내에서 mint() 직후 연속 호출. 충전소별·지역별 수익 실시간 집계. STOPortfolio의 필수 데이터 소스. UUPS Proxy. |
...
| 8 | **ReputationRegistry** | ChargeTransaction, RevenueTracker, StationRegistry (+ STRIKON 운영 데이터) | Explore용 region reputation snapshot 저장소. Oracle 패턴. Phase 3 선택사항. 인터페이스: `contracts/contracts/interfaces/ops/IReputationRegistry.sol`, 구현: `contracts/contracts/ops/ReputationRegistry.sol`. |
...
graph TD
    ...
    subgraph "필수형 (Essential) — Phase 1-2"
        ...
        SR["<b>StationRegistry</b><br/><i>충전소·충전기 계층</i>"]
    ...
    subgraph "파생형 (Derived)"
        ...
        RR["ReputationRegistry<br/><i>Explore story substrate</i>"]
    ...
    SR -->|"isRegistered()"| CT
    SR -->|"isRegistered() + regionId 조회"| RT
    CT -->|"chargeTransactionId"| CR
    PR -->|"EFkw, AFEC, ..."| CR
    RT -->|"지역 수익 풀"| SP
    CT -->|"세션량·피크 시간대 파생"| RR
    RT -->|"정산 연속성·수익 안정성 파생"| RR
    CR -->|"reductionId[]"| CB
    CB -->|"batchId"| VR
    SR -->|"활성 충전기 비율·region 매핑"| RR
    SR -->|"region 매핑"| SP
    RSTOF -->|"deploy"| RSTO
    RSTO -->|"보유량"| SP
...
| ReputationRegistry | ChargeTransaction, RevenueTracker, StationRegistry | ← Explore story substrate |
...
**목표**: 충전기 SE 칩 공개키 등록, 충전소·충전기 계층 구조 온체인 확립.
...
2. **모든 온체인 충전소는 EnergyFi 소유**: 온체인에 등록되는 충전소는 모두 EnergyFi가 직접 소유·운영. 제3자 CPO는 STRIKON 소프트웨어만 사용하며 온체인에 등록되지 않음.

3. **수익 귀속 규칙**:
   ```
   온체인 충전소 → distributableKrw 100% → 해당 지역 STO 투자자 풀
   (CPO 소유 충전소는 온체인에 없음 — STRIKON 오프체인에서 별도 정산)
   ```

**함수 명세 및 Mock Oracle 상세**: [Phase 1 스펙](phase1-infra-spec.md)

**대시보드**: 지역별 인프라 현황 / SE 칩 등록 현황

**통합 테스트 (2026.03 완료):**
- [x] 충전소 등록 후 `getStationsByRegion()` 검증
...
**목표**: 결제 완료 시 충전 데이터 온체인 기록. 충전소별·지역별 누적 수익 실시간 조회. ChargeRouter로 mint + recordRevenue 원자성 보장.
...
| **타임라인** | 2026.04 ~ 06 |
| **온체인 의존성** | ChargeTransaction → DeviceRegistry (SE 서명 검증) + StationRegistry (stationId 검증) / RevenueTracker → StationRegistry (regionId 조회 + stationId 검증) / ChargeRouter → ChargeTransaction + RevenueTracker |
...
| 10 | Setup 데이터 규모 | **포괄적** (충전소 5 + 충전기 15 + SE칩 12 + 3개 지역) |
...
9. **Region attestation period ordering**: `RevenueTracker.getRegionAttestationPeriods(regionId)` 는 항상 `period_yyyyMM` 기준 `중복 없는 오름차순(oldest → newest)` 을 반환한다. 프론트는 마지막 항목을 latest finalized period로 안전하게 사용할 수 있다.
...
cpo_id (UUID)                                →  (온체인 전달 안 함 — STRIKON 오프체인 전용)
...
**Mock Oracle**: 10개 메뉴 (모의 세션 발생 / 대량 세션 / 충전소 월정산 / 충전소별 수익 / 지역 수익 / 세션 상세 / 정산 이력 / 실패 시나리오 테스트 / 월경계 테스트 / ERC-721 정보)

**대시보드**: 7개 화면 (실시간 충전 피드 / 충전소별 수익 / 지역별 수익 통계 / 월별 통계 / 정산 이력 / 지역 수익 / 세션 상세 조회)
...
- [ ] RevenueTracker: 모의 세션 100건 후 stationAccumulated 정확성, 지역별 수익 집계, claim() 후 pending=0, ZeroAmount revert, 미등록 stationId revert, UUPS upgrade 권한
...
### Phase 3: RegionSTO + RegionSTOFactory + STOPortfolio + ReputationRegistry (STO 발행 + Explore)
...
**비즈니스 제약:**
- 온체인에 등록된 모든 충전소가 STO 참여 대상 (모두 EnergyFi 소유).
...
**Explore reputation 규격과 구현**:
- `ReputationRegistry`는 [`IReputationRegistry.sol`](../contracts/interfaces/ops/IReputationRegistry.sol) 을 단일 공개 규격으로 사용한다.
- concrete implementation은 [`ReputationRegistry.sol`](../contracts/ops/ReputationRegistry.sol) 에 있으며, `UUPS + AccessControl + BridgeGuarded + Pausable` 패턴을 따른다.
- 상세 metric 정의, period 규칙, source mapping은 [Phase 3 Reputation 스펙](phase3-reputation-spec.md) 참조.
- v1 범위는 `운영 신뢰도 + 수익 리듬 + 입지 성격` 3축이며, 사용자-facing narrative는 프론트가 조합한다.
...
| **3** | CCIPRevenueSender [EnergyFi L1 신규] / RegionSTO·Factory·Portfolio [KSD 체인, 보류] / ReputationRegistry(선택) | 파생형 | 미정 | 2027.01~ | 전자증권법 시행 + Explore reputation substrate 정의 |
...
| ✅ ~~StationRegistry 미등록 충전소에서 mint() 호출~~ | ~~🟡 High~~ | **해결됨**: ChargeTransaction.mint()와 RevenueTracker.recordRevenue() 모두 StationRegistry.isRegistered(stationId) 검증. 미등록 시 `StationNotRegistered` revert. |
